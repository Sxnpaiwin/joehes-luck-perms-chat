...
public final class ItemPlaceholder {
     ...
     */
    public static Component apply(JoehesLPC plugin, Player player, Component rendered, boolean withHover) {
        if (!plugin.getConfig().getBoolean("use-item-placeholder", false)
                || !player.hasPermission("JoehesLPC.itemplaceholder")) {
            return rendered;
        }
    ...
    private static Component displayName(JoehesLPC plugin, ItemStack item) {
        ...
        if (meta != null && meta.hasDisplayName()) {
            if (plugin.isPaper()) {
                ...
                return JoehesLPC.getLegacySerializer().deserialize(meta.getDisplayName());
            }
        }
